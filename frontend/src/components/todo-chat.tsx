...
const TodoChat: React.FC<TodoChatProps> = ({ onTaskOperation }) => {
  ...
  useEffect(() => {
    if (!user && !guestUserId) {
      ...
      const newGuestUserId = `guest_${timestamp}_${randomNum}`;
      setGuestUserId(newGuestUserId);
    }
  }, [user, guestUserId]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Determine user ID and token based on authentication status
      ...
      if (user) {
        ...
        actualUserId = user.id.toString();
      } else {
        // Guest user
        if (!guestUserId) {
          throw new Error("Guest user ID not generated");
        }
        actualUserId = guestUserId;
        token = null; // No token needed for guest users
      }

      const response = await sendMessage(
        inputMessage,
        actualUserId,
        token || "", // Pass empty string if no token (for guest users)
